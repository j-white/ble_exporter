...
/**
 * Helper class for loading the tinyb Java native library.
 *
 * (Shameless copied from TensorFlow)
 *
 * <p>The Java tinyb bindings require a native (JNI) library. This library
 * (libtinyb.so on Linux, libtinyb.dylib on OS X, libtinyb.dll on Windows)
 ...
 * set of ClassLoader resources (under org/opennms/tinyb/native/OS-ARCH).
 */
final class NativeLibrary {
    private static final boolean DEBUG =
            System.getProperty("org.opennms.iot.NativeLibrary.DEBUG") != null;
    private static final String JNI_LIBNAME = "javatinyb";
    private static final String LIBNAME = "tinyb";
    ...
    public static void load() {
        ...
        if (jniResource == null) {
            throw new UnsatisfiedLinkError(
                    String.format(
                            "Cannot find tinyb native library for OS: %s, architecture: %s. See "
                                    + "https://github.com/j-white/musecore/tree/master/README.md"
                                    ...
                                    + " org.opennms.tinyb.NativeLibrary.DEBUG=1 to the system properties of the JVM.",
                            os(), architecture()));
        }
        try {
            ...
            // load tinyb too
            final String libName = System.mapLibraryName(LIBNAME);
            final String resourceName = makeResourceName(libName);
            log("resourceName: " + resourceName);
            final InputStream resource = NativeLibrary.class.getClassLoader().getResourceAsStream(resourceName);
    ...
    private static void log(String msg) {
        if (DEBUG) {
            System.err.println("org.tinyb.NativeLibrary: " + msg);
        }
    }

    private static String makeResourceName(String baseName) {
        return "org/opennms/iot/native/" + String.format("%s-%s/", os(), architecture()) + baseName;
    }
    ...
    private static File createTemporaryDirectory() {
        ...
        String directoryName = "tinyb_native_libraries-" + System.currentTimeMillis() + "-";
        ...
        throw new IllegalStateException(
                "Could not create a temporary directory (tried to make "
                        + directoryName
                        + "*) to extract tinyb native libraries.");
    }
